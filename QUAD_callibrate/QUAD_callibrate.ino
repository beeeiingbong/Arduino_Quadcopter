  ...
  if(data == 'a'){
    ...
    if(cal_int != 2000){
      ...
      for (cal_int = 0; cal_int < 2000 ; cal_int ++){                                   //Take 2000 readings for calibration.
        ...
        acc_axis_cal[1] += acc_axis[1];                                                 //x axis force
        acc_axis_cal[2] += acc_axis[2];                                                 //y axis force
        acc_axis_cal[3] += acc_axis[3];                                                 //z axis force
      ...
      acc_axis_cal[1]  /=2000;                                                           //Divide the x axis by 2000
      acc_axis_cal[2]  /=2000;                                                           //Divide the y axis by 2000
      acc_axis_cal[3]  /=2000;                                                          //Divide the z axis by 2000
      ...
      Serial.print(" YAW ACC :");
      Serial.println(acc_axis_cal[3]);
      ...
      angle_yaw += gyro_yaw * 0.0000611;                                               //Calculate the travelled yaw angle
...
void gyro_signalen(){
  ...
  if(eeprom_data[28] & 0b10000000)gyro_pitch *= -1;               //Invert gyro_roll if the MSB of EEPROM bit 28 is set.
  gyro_pitch = gyro_axis[eeprom_data[29] & 0b00000011];          //Set gyro_pitch to the correct axis that was stored in the EEPROM.
  if(eeprom_data[29] & 0b10000000)gyro_roll *= -1;              //Invert gyro_pitch if the MSB of EEPROM bit 29 is set.
